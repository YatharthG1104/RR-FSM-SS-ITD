...
public class RR_Tele_Test extends OpMode {
    ...
    private FSM_TeleOps.LiftState CurrentState;
   ...
   // RevColorSensorV3 colorsensor;
    ...
    public void init() {
        ...
        CurrentState = FSM_TeleOps.LiftState.RegTele;
        EmergencyStop = false;
    }
       ...
       // move();
        ...
        switch (CurrentState) {
            case RegTele:
              //  move();
                ...
                if(gamepad1.right_bumper){
                    CurrentState = FSM_TeleOps.LiftState.SubmersibleReady;
                } else
                if(gamepad1.left_bumper){
                    CurrentState = FSM_TeleOps.LiftState.SamplePicked;
                } else
                if(gamepad2.dpad_left) {
                    CurrentState = FSM_TeleOps.LiftState.TransferBasket;
                } else
                if (gamepad2.b) {
                    CurrentState = FSM_TeleOps.LiftState.PickSpecimen;
                } else
                if (gamepad2.x) {
                    CurrentState = FSM_TeleOps.LiftState.SpecimenHangReady;
                } else
                if(gamepad2.dpad_right) {
                    CurrentState = FSM_TeleOps.LiftState.SpecimenHanged;
                } else
                if(gamepad1.back) {
                    CurrentState = FSM_TeleOps.LiftState.Stop;
                } else
                if(gamepad2.start) {
                    CurrentState = FSM_TeleOps.LiftState.Begin;
                }
                telemetry.update();
                break;
            ...
            case SubmersibleReady:
              //  move();
                Actions.runBlocking(new ParallelAction(
                        new FSM_TeleOps.MotorAction2(FrontSlide, Front_Slide_Intake_Enc,Front_Slide_Extend_Power),
                        new FSM_TeleOps.DoubleServoAction(TwistLeft, TwistRight, TwistL_Intake_Pos, TwistR_Intake_Pos),
                        new FSM_TeleOps.DoubleServoAction(ElbowLeft, ElbowRight, ElbowL_Transfer_Pos, ElbowR_Transfer_Pos),
                        new FSM_TeleOps.ServoAction(FrontClaw, FrontClaw_Open_Pos)
                ));
                telemetry.update();
                CurrentState = FSM_TeleOps.LiftState.RegTele;
                break;
             ...
             //   move();
                Actions.runBlocking(new ParallelAction(
                        new SequentialAction(
                                new ParallelAction(
                                        new FSM_TeleOps.DoubleServoAction(TwistLeft, TwistRight, TwistL_IntakeMiddle_Pos, TwistR_IntakeMiddle_Pos),
                                        new FSM_TeleOps.MotorAction2(FrontSlide, Front_Slide_Transfer_Enc,Front_Slide_Retract_Power)
                                ),
                                new FSM_TeleOps.DoubleServoAction(TwistLeft, TwistRight, TwistL_Transfer_Pos, TwistR_Transfer_Pos)
                        ),
                        new FSM_TeleOps.DoubleServoAction(ElbowLeft, ElbowRight, ElbowL_Transfer_Pos, ElbowR_Transfer_Pos),
                        new FSM_TeleOps.ServoAction(Claw,Claw_Open_Pos)
                ));
                telemetry.update();
                CurrentState = FSM_TeleOps.LiftState.RegTele;
                break;
            ...
            case TransferBasket:
               // move();
                Actions.runBlocking(new SequentialAction(
                        new FSM_TeleOps.ServoAction(Claw,Claw_Close_Pos),
                        new FSM_TeleOps.DoubleMotorAction(deliveryArmLeft, deliveryArmRight, Delivery_Arm_BasketReady_Enc, -Delivery_Arm_BasketReady_Enc,Delivery_Arm_Extend_Power, Delivery_Arm_Extend_Power),
                        new FSM_TeleOps.DoubleServoAction(ElbowLeft, ElbowRight, ElbowL_Basket_Pos, ElbowR_Basket_Pos),
                        new SleepAction(1),
                        new FSM_TeleOps.ServoAction(Claw,Claw_Open_Pos),
                        new ParallelAction(
                                new FSM_TeleOps.MotorAction2(deliveryArmLeft,Delivery_Arm_Transfer_Enc, Delivery_Arm_Retract_Power),
                                new FSM_TeleOps.MotorAction2(deliveryArmRight,-Delivery_Arm_Transfer_Enc, Delivery_Arm_Retract_Power),
                                new FSM_TeleOps.DoubleServoAction(ElbowLeft, ElbowRight, ElbowL_Transfer_Pos, ElbowR_Transfer_Pos),
                                new FSM_TeleOps.MotorAction2(FrontSlide, Front_Slide_Resting_Enc,Front_Slide_Retract_Power)
            ...
            case PickSpecimen:
                ...
                Actions.runBlocking(new ParallelAction(
                        new ParallelAction(
                                new FSM_TeleOps.MotorAction2(deliveryArmLeft,Delivery_Arm_Resting_Enc, Delivery_Arm_Retract_Power),
                                new FSM_TeleOps.MotorAction2(deliveryArmRight,-Delivery_Arm_Resting_Enc, Delivery_Arm_Retract_Power)
                        ),
                        new FSM_TeleOps.DoubleServoAction(ElbowLeft, ElbowRight, ElbowL_Intake_Pos, ElbowR_Intake_Pos),
                        new FSM_TeleOps.ServoAction(Wrist, Wrist_Intake_Pos),
                        new FSM_TeleOps.ServoAction(Claw,Claw_Open_Pos),
                        new FSM_TeleOps.MotorAction2(FrontSlide, Front_Slide_Resting_Enc,Front_Slide_Retract_Power)
                ));
                telemetry.update();
            ...
            case SpecimenHangReady:
               // move();
                Actions.runBlocking(new SequentialAction(
                                ...
                                new ParallelAction(
                                        new ParallelAction(
                                                new FSM_TeleOps.MotorAction2(deliveryArmLeft,Delivery_Arm_HangReady_Enc, Delivery_Arm_Extend_Power),
                                                new FSM_TeleOps.MotorAction2(deliveryArmRight,-Delivery_Arm_HangReady_Enc, Delivery_Arm_Extend_Power)
                                        ),
                                        new FSM_TeleOps.DoubleServoAction(ElbowLeft, ElbowRight, ElbowL_Hang_Pos, ElbowR_Hang_Pos),
                                        new FSM_TeleOps.ServoAction(Wrist, Wrist_Hang_Pos),
                                        new FSM_TeleOps.MotorAction2(FrontSlide, Front_Slide_Resting_Enc,Front_Slide_Retract_Power)
                                )
                        )
                );
            ...
            case SpecimenHanged:
               // move();
                Actions.runBlocking(new SequentialAction(
                                new ParallelAction(
                                        new ParallelAction(
                                                new FSM_TeleOps.MotorAction2(deliveryArmLeft,Delivery_Arm_HangDone_Enc, Delivery_Arm_Retract_Power),
                                                new FSM_TeleOps.MotorAction2(deliveryArmRight,-Delivery_Arm_HangDone_Enc, Delivery_Arm_Retract_Power)
                                        ),
                                        new FSM_TeleOps.DoubleServoAction(ElbowLeft, ElbowRight, ElbowL_HangDone_Pos, ElbowR_HangDone_Pos),
                                        new FSM_TeleOps.MotorAction2(FrontSlide, Front_Slide_Resting_Enc,Front_Slide_Retract_Power)
                                ),
                                new FSM_TeleOps.ServoAction(Claw, Claw_Open_Pos)
                        )
            ...
            case Begin:
                Actions.runBlocking(
                        new SequentialAction(
                                new FSM_TeleOps.DoubleMotorAction(deliveryArmLeft,deliveryArmRight,Delivery_Arm_Transfer_Enc, Delivery_Arm_Transfer_Enc, Delivery_Arm_Extend_Power, Delivery_Arm_Extend_Power),
                                new FSM_TeleOps.DoubleServoAction(ElbowLeft,ElbowRight,ElbowL_Transfer_Pos,ElbowR_Transfer_Pos),
                                new FSM_TeleOps.ServoAction(Claw,Claw_Open_Pos)
                        )
                );
